    ...
    >
      <Stack direction={{ xs: 'column', md: 'row' }} spacing={1} sx={{ alignItems: { xs: 'stretch', md: 'center' }, justifyContent: 'space-between' }}>
        ...
        <Stack direction="row" spacing={1} sx={{ flexWrap: 'wrap', rowGap: 1, justifyContent: 'flex-end' }}>
          {selectedCount > 0 ? (
            <>
              <Button startIcon={<DownloadRoundedIcon />} variant="contained" onClick={onDownloadSelected}>下载所选</Button>
              <Button startIcon={<ClearRoundedIcon />} color="inherit" onClick={onClearSelection}>清空选择</Button>
            </>
          ) : (
