export const PORTRA_SURFACE = {
  background: '#e6e2e0',
  paper: '#f8f3eb',
  paperMuted: '#ebe6dd',
  paperSoft: '#f0ece5',
  surface: '#f0ece5',
  inset: '#f0ece5',
  stone: '#e6e2e0',
  stoneDeep: '#d8d3cf',
  ink: '#111015',
  subInk: '#151318',
  muted: '#5d6167',
  faint: '#858087',
  portraBlue: '#0d2fb2',
  portraBlueHover: '#09258f',
  portraBlueSoft: '#e7ebfa',
  filmYellow: '#f7ce3a',
  filmYellowSoft: '#fff6d6',
  warmOrange: '#f85104',
  warmOrangeDeep: '#f85104',
  warmOrangeSoft: '#fae8df',
  borderSubtle: 'rgba(21,19,24,.12)',
  borderSoft: 'rgba(21,19,24,.08)',
  borderDashed: 'rgba(13,47,178,.15)'
}

export const PORTRA_RADIUS = {
  compact: '999px',
  control: '14px',
  card: '20px',
  panel: '24px',
  shell: '28px'
}

export const PORTRA_SHADOW = {
  hairline: '0 1px 0 rgba(255, 255, 255, 0.62) inset',
  soft: '0 16px 40px rgba(21,19,24,.09)',
  floating: '0 22px 54px rgba(21,19,24,.12)',
  hover: '0 6px 20px rgba(13,47,178,.10)'
}

export const PORTRA_LAYOUT = {
  pageMaxWidth: 1360,
  workflowMaxWidth: 1360,
  wideWorkflowMaxWidth: 1440,
  galleryMaxWidth: 1320,
  pageGutter: 'clamp(20px, 4vw, 48px)',
  pageGutterTotal: 'clamp(40px, 8vw, 96px)',
  mobileGutterTotal: '24px',
  contentTopGap: 24,
  sectionGap: 24,
  workbenchHeight: {
    xs: 'calc(100dvh - 212px)',
    md: 'calc(100dvh - 154px)'
  },
  rightPanelWidth: {
    lg: '320px',
    xl: '320px'
  },
  compactRightPanelWidth: {
    lg: '300px',
    xl: '320px'
  },
  orderSidebarWidth: {
    md: '320px',
    xl: '328px'
  }
}

export const PORTRA_ARCHIVE_SURFACE_SX = {
  bgcolor: PORTRA_SURFACE.paper,
  borderColor: PORTRA_SURFACE.borderSubtle,
  boxShadow: PORTRA_SHADOW.soft
}
