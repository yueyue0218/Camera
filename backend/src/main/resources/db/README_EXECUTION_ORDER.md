...
5. 执行 `add_user_profile_visibility.sql`。该脚本会先检查 `information_schema.COLUMNS`，字段已存在时只输出提示，不重复添加。
6. 执行 `SELECT order_id, COUNT(*) FROM payment_records GROUP BY order_id HAVING COUNT(*) > 1;`。如有结果，先依据权威支付记录人工处理，不得删除后直接继续。
7. 确认无重复后执行 `add_payment_order_unique_constraint.sql`。
8. 执行 `add_dispute_previous_order_status.sql`，检查脚本最后返回的 unresolved dispute；无法回填的历史记录保留 nullable，由新代码安全拒绝恢复。
9. 检查所有迁移均成功且没有半迁移状态，再 push `main` 部署代码。
10. 部署后验证重复支付、争议驳回恢复、公开作品和交付文件权限。
