...
| 9 | `migration/add_user_profile_visibility.sql` | users 资料展示字段（当前 baseline 尚未包含）；MySQL 8 兼容、可重复执行，字段已存在时只输出提示 |
...
1. 备份数据库。
2. 执行 `add_user_profile_visibility.sql`。该脚本会先检查 `information_schema.COLUMNS`，字段已存在时只输出提示，不重复添加。
